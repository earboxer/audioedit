...
static int
wav2format(const wavheader_t * wav)
{
    switch (wav->format) {
    case 1:
    case -2:
    ...
    case 17:
        return wav->bitdepth == 4 ? WAV_FMT_IMA_ADPCM : -1;
    default:
        return -1;
    }
}
 ...
 */
static int
endian2h(const char fmt[], void *p)
{
...
static int
wav_setdev(const wav_info_t * info)
{
    if (info->devformat < 0)
